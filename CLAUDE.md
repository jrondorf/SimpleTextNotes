...
SimpleTextNotes/                 # Main app target
  ...
  *.lproj/Localizable.strings    # 23 languages, 64 keys each (en is the source language)
...
- 23 languages under `SimpleTextNotes/*.lproj/Localizable.strings`, 64 keys each; `en` is the
  development language and the source of truth. `SWIFT_EMIT_LOC_STRINGS = YES`.
